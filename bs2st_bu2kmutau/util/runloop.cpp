...
int main(int argc, char * argv[]) {
  ...
  const char * outfile = parser.getCmdOption("-o", "bs2st2buk_kmux.root");

  std::string dirs = parser.getCmdOption("-d", "Bss2BuK_KMuX_Tuple,Bss2BuKSS_KMuX_Tuple");
  ...
  for( std::vector<std::string>::const_iterator it=dirlist.begin(); it != dirlist.end(); ++it ) {
    ...
    bs2st_bu2kmutau::bs2st2buk_kmux_loop l(tree,&fout, *it );
