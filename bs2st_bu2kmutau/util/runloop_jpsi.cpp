...
int main(int argc, char * argv[]) {
  ...
  const char * outfile = parser.getCmdOption("-o", "bs2st2buk_jpsik.root");

  std::string dirs = parser.getCmdOption("-d", "Bss2BuK_Tuple,Bss2BuKSS_Tuple");
  ...
  for( std::vector<std::string>::const_iterator it=dirlist.begin(); it != dirlist.end(); ++it ) {
    ...
    bs2st_bu2kmutau::bs2st2buk_jpsik_loop l(tree,&fout, *it );
